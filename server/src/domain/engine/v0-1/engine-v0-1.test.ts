/**
 * EngineV01 tests — Phase 6F (throwaway v0.1).
 *
 * Tests the public surface of EngineV01 in isolation: dispatch, composition,
...
import { EngineV01 } from './engine-v0-1.js';
...
describe('EngineV01', () => {
  ...
  it('baseline token.move is accepted and broadcasts a token.moved event', async () => {
    const engine = await EngineV01.open(world.campaignId, world.storage);
  ...
  it('throws at load time when two rulesets register a merger for the same action', async () => {
    ...
    await expect(
      EngineV01.open(world.campaignId, world.storage, [
        dndRuleset,
        secondRuleset,
      ]),
  ...
  it('token.linkToActor as GM updates token actorId and emits token.linked event', async () => {
    ...
    const engine2 = await EngineV01.open(world.campaignId, world.storage);
